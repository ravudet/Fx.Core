...
namespace Fx
{
        ...
        where TVerifier : IVerifier, new()
    {
        ...
        {
            if (BaseIsCompilerDiagnosticIncluded(diagnostic, compilerDiagnostics))
            {
                return true;
            }
        ...
        protected AnalyzerTest(string first)
        {
        ...
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> that the operation will observe.</param>
        /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
        protected virtual async Task BaseRunImplAsync(CancellationToken cancellationToken)
        {
            Verify.NotEmpty($"{nameof(TestState)}.{nameof(SolutionState.Sources)}", TestState.Sources);
        ...
        protected virtual bool BaseIsCompilerDiagnosticIncluded(Diagnostic diagnostic, CompilerDiagnostics compilerDiagnostics)
        {
    ...
    public enum CodeFixTestBehaviors
    {
        ...
        SkipFixAllInDocumentCheck = 1,
        //
        // Summary:
        //     Skip the Fix All in Project check.
        SkipFixAllInProjectCheck = 2,
        //
        // Summary:
        //     Skip the Fix All in Solution check.
        SkipFixAllInSolutionCheck = 4,
        //
        // Summary:
    ...
    [Obsolete("Use CodeActionValidationMode instead.")]
    ...
    public static class Formatter
    {
        ...
        //
        //   options:
        //     An optional set of formatting options. If these options are not supplied the
        //     current set of options from the document's workspace will be used.
        //
        //   cancellationToken:
        ...
        internal static async Task<Document> FormatAsync(Document document, SyntaxAnnotation annotation, OptionSet options, IEnumerable<string> rules, CancellationToken cancellationToken)
        {
            if (document == null)
            {
            ...
            return document.WithSyntaxRoot(await document.GetSyntaxRootAsync(cancellationToken).ConfigureAwait(continueOnCapturedContext: false));
        }
