namespace Microsoft.CodeAnalysis.Testing
{
    ...
    //// TODO maybe you should create an `iassert` interface in fx.core and then have an `iverifier` implementation that adapts an `iassert` and have `fx.test` have an `iassert` implementation for mstest; you would do this because right now this project has 2 dependencies, which is nice for demonstrating how that would work, but not great otherwise //// TODO i'm not sure this is a good choice because the interface isn't as future-proof, and basically becomes at least somewhat coupled with `iverifier`
